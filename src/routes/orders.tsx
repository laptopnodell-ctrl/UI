...
export const Route = createFileRoute("/orders")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "See your past Vino Tasty Hub orders, reorder favourites and track deliveries.",
      },
      { property: "og:title", content: "My orders — Vino Tasty Hub" },
      {
        property: "og:description",
        content: "Your order history and live deliveries in one place.",
      },
    ],
  }),
  component: Orders,
  ...
  return (
    <Screen>
      ...
      <div className="flex flex-col gap-3 px-4 pt-4">
        {orders.map((o) => {
          const isActive =
            o.status === "preparing" || o.status === "order-placed" || o.status === "on-the-way";
          return (
            <article key={o.id} className="vino-card p-4">
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`rounded-full px-2.5 py-1 text-[11px] font-bold ${statusStyle[o.status]}`}
                >
